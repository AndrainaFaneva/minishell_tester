...
test_from_file() {
	...
	do
		...
		echo -n "$INPUT" | $MINISHELL_PATH/$EXECUTABLE 2>tmp_err_minishell >tmp_out_minishell
			...
			< tmp_out_minishell grep -v "$MINISHELL_PROMPT" | grep -v  "$HEREDOC_PROMPT" > out_minishell
...
test_leaks() {
	...
	do
		...
		if [[ $line == \#* ]] || [[ $line == "" ]] ; then
			...
			exit_bash=$?
			echo -ne "\033[1;34mSTD_OUT:\033[m "
			cat tmp_out_minishell | grep -v "minishell>" | grep -v "heredoc> " > tmp_out_minishell_2
			if ! diff -q tmp_out_minishell_2 tmp_out_bash >/dev/null ;
			then
				echo -ne "❌  " | tr '\n' ' '
				((TEST_KO_OUT++))
				((FAILED++))
			...
			echo -n "$INPUT" | valgrind --suppressions=readline.supp --leak-check=full --show-leak-kinds=all --track-origins=yes --verbose --log-file=tmp_valgrind-out.txt $MINISHELL_PATH/$EXECUTABLE 2>/dev/null >/dev/null
			...
			reachable=$(cat tmp_valgrind-out.txt | grep "still reachable:" | awk 'END{print $4}')
			...
			if [ "$reachable" != "0" ] || [ "$definitely_lost" != "0" ] || [ "$possibly_lost" != "0" ] || [ "$indirectly_lost" != "0" ] && [[ -z "$all_blocks_freed" ]];
			then
				echo -ne "❌ "
				((LEAKS++))
			else
